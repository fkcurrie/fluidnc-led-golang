...
// loadSVG loads and rasterizes an SVG file to the LED matrix size
func loadSVG(svgPath string) (image.Image, error) {
	// Read the SVG file
	...
	// Parse the SVG data
	icon, err := oksvg.ReadIconStream(svgData, oksvg.IgnoreErrorMode)
	if err != nil {
		return nil, err
	}

	// Set the SVG to fit within our LED matrix size
	icon.SetTarget(0, 0, float64(DISPLAY_WIDTH), float64(DISPLAY_HEIGHT))

	// Create an image to rasterize to
	img := image.NewRGBA(image.Rect(0, 0, DISPLAY_WIDTH, DISPLAY_HEIGHT))

	// Create a rasterizer
	scanner := rasterx.NewScannerGV(DISPLAY_WIDTH, DISPLAY_HEIGHT, img, img.Bounds())
	raster := rasterx.NewDasher(DISPLAY_WIDTH, DISPLAY_HEIGHT, scanner)
...
// renderSVG renders an image to the LED matrix
func renderSVG(frameData [][]byte, img image.Image) {
	// Clear the frame data
	for row := range frameData {
		for col := 0; col < DISPLAY_WIDTH; col++ {
			idx := col * 6
	...
	// Convert the image to the LED matrix format
...
func main() {
	...
	svgFile := flag.String("svg", "", "Path to SVG file to display")
	flag.Parse()
	...
	var svgImage image.Image
	if *svgFile != "" {
		img, err := loadSVG(*svgFile)
		if err != nil {
			log.Printf("Failed to load SVG: %v", err)
		} else {
			svgImage = img
			log.Printf("Loaded SVG file: %s", *svgFile)
		}
	}
